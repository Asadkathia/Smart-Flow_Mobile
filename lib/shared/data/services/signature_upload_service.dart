...
class SignatureUploadService {
  ...
  SignatureUploadService(this._mediaUploadService);
  ...
  void _validateSignatureSize(File signature) {
    ...
    if (fileSize > AppConstants.maxSignatureSize) {
      throw ValidationException(
        message:
            'Signature file size exceeds maximum allowed size of ${AppConstants.maxSignatureSizeMb}MB.',
  ...
  Future<String> uploadSignature(File signatureFile, String visitId) async {
    try {
  ...
  Future<void> deleteSignature(String signaturePath, String visitId) async {
    try {
...
final signatureUploadServiceProvider = Provider<SignatureUploadService>((ref) {
  ...
  return SignatureUploadService(mediaUploadService);
});
