...
class RateLimiterService {
  ...
  static const String _aiRequestsKey = 'ai_requests';
  ...
  Future<bool> isRequestAllowed(String key) async {
    ...
    final validTimestamps = timestamps.where((ts) => ts.isAfter(windowStart)).toList();
  ...
  Future<void> recordRequest(String key) async {
    ...
    final isoStrings = validTimestamps.map((ts) => ts.toIso8601String()).toList();
    await _box?.put(key, isoStrings);
    
    Logger.debug('Rate limiter: Recorded request for $key. Count: ${validTimestamps.length}/$_maxRequestsPerHour');
  }
