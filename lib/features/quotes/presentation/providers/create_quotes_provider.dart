...
class CreateQuotes extends _$CreateQuotes {
  ...
  Future<void> loadMaterials() async {
    try {
      ...
      final materials = inventoryItems.map((item) => MaterialItem(
        name: item.name,
  ...
  Future<String?> saveQuote({required String visitId}) async {
    ...
    try {
      ...
      if (isEditing) {
        print('[CreateQuotes] Calling quoteRepository.updateQuote for: \${quote.id}');
        await quoteRepository.updateQuote(quote);
      } else {
        print('[CreateQuotes] Calling quoteRepository.createQuote for: \${quote.id}');
    ...
    for (var item in quote.lineItems) {
      if (item.type == LineItemType.service_call_fee) {
        serviceCallFeeLine = ServiceLine(
          service: ServiceItem(name: item.description, pricePerUnit: item.unitPrice),
          quantity: item.qty,
        );
