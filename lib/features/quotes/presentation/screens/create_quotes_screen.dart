...
class CreateQuotesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreateQuotesScreen> createState() => _CreateQuotesScreenState();
}

class _CreateQuotesScreenState extends ConsumerState<CreateQuotesScreen> {
  ...
  void didChangeDependencies() {
    ...
    _initialized = true;
    Future.microtask(_initializeEditorState);
  }

  Future<void> _initializeEditorState() async {
    if (!mounted) return;

    final notifier = ref.read(createQuotesProvider.notifier);
    notifier.reset();

    try {
      if (widget.mode == 'edit' && widget.quoteId != null) {
        final quote = await ref
            .read(quoteRepositoryProvider)
            .getQuote(widget.quoteId!);
        if (!mounted) return;
        notifier.loadQuote(quote);
      } else if (widget.mode == 'revision' && widget.sourceQuoteId != null) {
        final quote = await ref
            .read(quoteRepositoryProvider)
            .getQuote(widget.sourceQuoteId!);
        if (!mounted) return;
        notifier.loadQuoteForRevision(quote);
      }
    } catch (e) {
      if (!mounted) return;
      context.showErrorSnackBar('Failed to load quote context: $e');
    }
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            // SERVICES Section
            CreateQuotesSectionHeader(title: 'SERVICES'),
            Column(
              children: [
                ...quoteState.serviceLines.map((line) {
                  final isServiceCallFee = line.service.name
                      .toLowerCase()
                      .contains('service call fee');
            ...
            Column(
              children: [
                ...quoteState.materialLines.map(
                  (line) => CreateQuotesLineItem(
                    ...
                    onTap: () =>
                        _showMaterialQuantityEditor(context, ref, line),
                  ),
                ),
                CreateQuotesAddButton(
                  label: 'Add Materials',
            ...
            // Notes Section
            CreateQuotesSectionHeader(title: 'NOTES'),
            ...
            // Terms Section
            CreateQuotesSectionHeader(title: 'TERMS'),
            CreateQuotesMessageRow(
            ...
            CreateQuotesSectionHeader(title: 'EXPIRATION DATE'),
            GestureDetector(
              onTap: () async {
            ...
            CreateQuotesSectionHeader(title: 'ESTIMATE MESSAGE'),
            CreateQuotesMessageRow(
  ...
  void _handleDone(BuildContext context, WidgetRef ref) async {
    ...
    final totalLineItems =
        quoteState.serviceLines.length + quoteState.materialLines.length;
    ...
    if (!validation.isValid) {
      if (context.mounted) {
        context.showErrorSnackBar(
          validation.errorMessage ?? 'Cannot finalize quote',
        );
      }
      return;
    }
    ...
    if (widget.visitId.isEmpty) {
      if (context.mounted) {
        context.showErrorSnackBar(
          'Visit ID is required. Please navigate from a visit.',
        );
      }
      return;
    }
      ...
      builder: (dialogContext) => CreateQuotesServicePickerDialog(
        ...
        onAdd: () {
          ...
          if (name.isNotEmpty && price > 0 && quantity > 0) {
            ref
                .read(createQuotesProvider.notifier)
                .addCustomService(name, price, quantity);
            CustomToast.success('Added $name');
            Navigator.pop(dialogContext);
          } else {
            CustomToast.error(
              'Please enter valid service name, price, and quantity',
            );
          }
        },
      ),
  ...
  void _showServiceQuantityEditor(
  ...
  void _showMaterialQuantityEditor(
    BuildContext context,
    WidgetRef ref,
    MaterialLine line,
  ) {
  ...
  void _showMessageEditor(BuildContext context, WidgetRef ref) {
    ...
    final messageController = TextEditingController(
      text: quoteState.estimateMessage,
    );
